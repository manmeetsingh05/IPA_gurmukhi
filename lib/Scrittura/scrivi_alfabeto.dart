...
import 'dart:math'; // Necessario per la conversione dei checkpoint
...
class _PunjabiTracingPageState extends State<PunjabiTracingPage> {
  ...
  Future<void> _loadLetterData() async {
    if (_currentLetter.isEmpty) {
       setState(() { _isLoading = false; _errorMessage = "Indice lettera non valido."; }); return;
    }
    final String assetPath = 'assets/png/$_currentLetter.png';
    setState(() {
      _isLoading = true; _errorMessage = null; _letterImage = null;
      _letterMask = null; _letterBounds = null;
    ...
    try {
      ...
      _maskHeight = _letterImage!.height;
       ...
       if (_maskWidth <= 0 || _maskHeight <= 0) throw Exception("Dimensioni immagine non valide.");

      final ByteData? pixels = await _letterImage!.toByteData(format: ui.ImageByteFormat.rawRgba);
      if (pixels != null) {
      ...
      } else { throw Exception("Impossibile leggere i pixel."); }

      _currentCheckpoints = letterCheckpoints[_currentLetter] ?? [];
      if (!letterCheckpoints.containsKey(_currentLetter)) {
         _errorMessage = "Definizione checkpoint mancante per '$_currentLetter'.";
         print(_errorMessage);
  ...
  void _scaleCheckpoints() {
    if (!mounted || _letterBounds == null || _letterImage == null || _currentCheckpoints.isEmpty) {
      if (_scaledCheckpoints.isNotEmpty) WidgetsBinding.instance.addPostFrameCallback((_) { if (mounted) setState(() => _scaledCheckpoints = []); });
      return;
    }
    ...
    final List<Rect> newScaledCheckpoints = _currentCheckpoints.map((r) => Rect.fromLTWH(
      _letterBounds!.left + (r.left * scaleX), _letterBounds!.top + (r.top * scaleY),
  ...
  bool _isInsideLetterBounds(Offset point, {int tolerance = 15}) {
    if (_letterBounds == null || _letterMask == null || _maskWidth == 0) return false;
    ...
    if (checkX >= 0 && checkX < _maskWidth && checkY >= 0 && checkY < _maskHeight) {
    ...
    for (int dx = -tolerance; dx <= tolerance; dx++) {
      for (int dy = -tolerance; dy <= tolerance; dy++) {
        ...
        if (tCheckX >= 0 && tCheckX < _maskWidth && tCheckY >= 0 && tCheckY < _maskHeight) {
  ...
  void _onPanStart(DragStartDetails details) {
    if (_isCompleted || _isLoading || _letterImage == null || _letterBounds == null) return;
  ...
  void _onPanUpdate(DragUpdateDetails details) {
    if (_isCompleted || _isLoading || _isOutOfBounds || _strokes.isEmpty) return;
  ...
  void _checkCheckpointHit(Offset position) {
    ...
    if (_scaledCheckpoints.isNotEmpty && _nextCheckpointIndex < _scaledCheckpoints.length) {
        ...
        if (_lastProcessedCheckpointIndex != _nextCheckpointIndex) {
          ...
          if (_nextCheckpointIndex == _scaledCheckpoints.length) {
            ...
            // completamente aggiornato prima di dichiarare vittoria
             WidgetsBinding.instance.addPostFrameCallback((_) {
                if(mounted && !_isCompleted && _nextCheckpointIndex == _scaledCheckpoints.length) {
                   _completeTrace();
                }
             });
          }
  ...
  // --- Dialoghi (Invariati) ---
  void _showErrorDialog(String message) { /* ... codice invariato ... */
    if (!mounted || !ModalRoute.of(context)!.isCurrent) return;
    showDialog(
      context: context, barrierDismissible: false, builder: (context) => AlertDialog(
        backgroundColor: Theme.of(context).colorScheme.surface, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(AppTheme.cardRadius)),
        contentPadding: const EdgeInsets.all(AppTheme.defaultPadding), content: Column(mainAxisSize: MainAxisSize.min, children: [
          Image.asset('assets/images/scrittura.png', height: 120), const SizedBox(height: 16),
          Text('Attenzione!', style: Theme.of(context).textTheme.titleLarge?.copyWith(color: Colors.orange.shade800, fontWeight: FontWeight.bold), textAlign: TextAlign.center),
          const SizedBox(height: 12), Text(message, style: Theme.of(context).textTheme.bodyMedium?.copyWith(color: Theme.of(context).colorScheme.onSurface), textAlign: TextAlign.center),
          const SizedBox(height: 20), ElevatedButton(
            onPressed: () => Navigator.of(context).pop(), style: ElevatedButton.styleFrom(backgroundColor: AppTheme.primaryColor, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(AppTheme.buttonRadius))),
            child: Text('Riprova', style: Theme.of(context).textTheme.labelLarge?.copyWith(color: Colors.white)),
          ),
        ]),
      ),
    );
   }
  void _showSuccessDialog() { /* ... codice invariato ... */
     if (!mounted || !ModalRoute.of(context)!.isCurrent) return;
    showDialog(
      context: context, barrierDismissible: false, builder: (context) => AlertDialog(
        backgroundColor: Theme.of(context).colorScheme.surface, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(AppTheme.cardRadius)),
        contentPadding: const EdgeInsets.all(AppTheme.defaultPadding), content: Column(mainAxisSize: MainAxisSize.min, children: [
          Image.asset('assets/images/giustoscrittura.png', height: 150), const SizedBox(height: 16),
          Text('Complimenti!', style: Theme.of(context).textTheme.titleLarge?.copyWith(color: AppTheme.accentColor, fontWeight: FontWeight.bold), textAlign: TextAlign.center),
          const SizedBox(height: 12), Text('Lettera completata correttamente!', style: Theme.of(context).textTheme.bodyMedium?.copyWith(color: Theme.of(context).colorScheme.onSurface), textAlign: TextAlign.center),
          const SizedBox(height: 20), ElevatedButton(
            onPressed: () {
              if (!mounted) return;
              Navigator.of(context).pop();
              if (_currentIndex < punjabiAlphabet.length - 1) {
                _currentIndex++; _loadLetterData();
              } else {
                if (Navigator.canPop(context)) Navigator.of(context).pop();
              }
            },
            style: ElevatedButton.styleFrom(backgroundColor: AppTheme.accentColor, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(AppTheme.buttonRadius))),
            child: Text(_currentIndex < punjabiAlphabet.length - 1 ? 'Prossima Lettera' : 'Fine', style: Theme.of(context).textTheme.labelLarge?.copyWith(color: Colors.white)),
          ),
        ]),
      ),
    );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBarTitle('Ricalca: ${_currentLetter.isNotEmpty ? _currentLetter : "..."}'),
      body: _buildBody(),
  ...
  void _resetCurrentAttempt() {
     ...
     setState(() { _resetTracingState(fullReset: _isCompleted); });
  }

  Widget _buildBody() {
    if (_errorMessage != null) return _buildErrorView();
    if (_isLoading || _letterImage == null) return Center(child: CircularProgressIndicator(color: AppTheme.secondaryColor));

    return LayoutBuilder(
      builder: (context, constraints) {
        final Size canvasSize = constraints.biggest;
        final Size imageSize = Size(_letterImage!.width.toDouble(), _letterImage!.height.toDouble());
        final FittedSizes fittedSizes = applyBoxFit(BoxFit.contain, imageSize, canvasSize);
        final Rect newLetterBounds = Alignment.center.inscribe(fittedSizes.destination, Offset.zero & canvasSize);
        ...
        WidgetsBinding.instance.addPostFrameCallback((_) {
           ...
           if (needsScaleUpdate || (_scaledCheckpoints.isEmpty && _currentCheckpoints.isNotEmpty)) {
              _scaleCheckpoints();
           }
        });

        if (_letterBounds == null) return Center(child: CircularProgressIndicator(color: AppTheme.secondaryColor));
            ...
            GestureDetector(
              ...
              child: CustomPaint(
                ...
                painter: TracingPainter(
                  strokes: _strokes, letterImage: _letterImage!, letterBounds: _letterBounds!,
                  tracePaint: _tracePaint, scaledCheckpoints: _scaledCheckpoints,
                  nextCheckpointIndex: _nextCheckpointIndex, checkpointPaint: _checkpointPaint,
                  nextCheckpointPaint: _nextCheckpointPaint, hitCheckpointPaint: _hitCheckpointPaint,
                  showCheckpoints: true, // !!! IMPOSTA A 'false' PER LA RELEASE !!!
                ),
              ),
            ),
            Positioned(top: 10, left: 10, child: _buildLetterAnimation()),
            if (_isCompleted) Positioned.fill(child: IgnorePointer(child: Container(
              color: AppTheme.accentColor.withOpacity(0.1), child: Center(
                child: Icon(Icons.check_circle, color: AppTheme.accentColor.withOpacity(0.6), size: 150, shadows: [Shadow(color: Colors.black.withOpacity(0.2), blurRadius: 8, offset: Offset(2, 2))]),
              ),
            ))),
          ],
        );
   ...
   Widget _buildErrorView() { /* ... codice invariato ... */
      return Center(
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text('Oops! Si è verificato un errore.', style: Theme.of(context).textTheme.titleLarge?.copyWith(color: Theme.of(context).colorScheme.error), textAlign: TextAlign.center),
            const SizedBox(height: 10),
            Text(_errorMessage ?? "Errore sconosciuto.", style: Theme.of(context).textTheme.bodyMedium, textAlign: TextAlign.center),
            const SizedBox(height: 30),
            ElevatedButton.icon(
              icon: Icon(Icons.refresh, color: Colors.white),
              label: Text('Riprova Caricamento', style: TextStyle(color: Colors.white)),
              onPressed: _loadLetterData,
              style: ElevatedButton.styleFrom(backgroundColor: AppTheme.primaryColor, padding: EdgeInsets.symmetric(horizontal: 20, vertical: 12)),
            ),
            const SizedBox(height: 10),
             TextButton(
  ...
  Widget _buildLetterAnimation() { /* ... codice invariato ... */
      String gifAssetPath = 'assets/gif/${_currentLetter.isNotEmpty ? _currentLetter : ''}.gif';
      return Container(
       padding: const EdgeInsets.all(4),
       decoration: BoxDecoration(
          color: Theme.of(context).colorScheme.surface.withOpacity(0.85),
          borderRadius: BorderRadius.circular(8),
          boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.15), blurRadius: 5, spreadRadius: 1, offset: Offset(1, 1))]
       ),
       child: Image.asset(
         gifAssetPath,
         ...
         errorBuilder: (context, error, stackTrace) => Container(
           ...
           child: Tooltip(message: 'GIF non trovata', child: Icon(Icons.image_not_supported_outlined, color: Theme.of(context).colorScheme.error.withOpacity(0.7), size: 30)),
         ),
         frameBuilder: (context, child, frame, wasSyncLoaded) => wasSyncLoaded ? child : AnimatedOpacity(
           opacity: frame == null ? 0 : 1, duration: const Duration(milliseconds: 350), curve: Curves.easeOut, child: child,
         ),
       ),
    );
   }

 bool listEquals<T>(List<T>? a, List<T>? b) { /* ... codice invariato ... */
    if (a == null) return b == null;
    if (b == null || a.length != b.length) return false;
    if (identical(a, b)) return true;
    for (int index = 0; index < a.length; index += 1) { if (a[index] != b[index]) return false; }
    return true;
 }
...
class TracingPainter extends CustomPainter {
  ...
  TracingPainter({
    ...
    required this.letterImage, required this.letterBounds, required this.tracePaint,
    required this.scaledCheckpoints, required this.nextCheckpointIndex,
    required this.checkpointPaint, required this.nextCheckpointPaint,
    required this.hitCheckpointPaint, this.showCheckpoints = false,
  });

  @override
  void paint(Canvas canvas, Size size) {
    // 1. Disegna immagine
    canvas.drawImageRect(letterImage, Rect.fromLTWH(0, 0, letterImage.width.toDouble(), letterImage.height.toDouble()), letterBounds, Paint()..filterQuality = FilterQuality.medium);

    // 2. Disegna checkpoint
    if (showCheckpoints) { /* ... codice invariato ... */
      for (int i = 0; i < scaledCheckpoints.length; i++) {
        final rect = scaledCheckpoints[i];
        Paint paintToUse;
        if (i < nextCheckpointIndex) paintToUse = hitCheckpointPaint;
        else if (i == nextCheckpointIndex) paintToUse = nextCheckpointPaint;
        else paintToUse = checkpointPaint;
    ...
    final circlePaint = Paint()..color = tracePaint.color..style = PaintingStyle.fill;
    for (final singleStroke in strokes) {
       if (singleStroke.isNotEmpty) {
          if (singleStroke.length > 1) {
             ...
             for (int i = 1; i < singleStroke.length; i++) { path.lineTo(singleStroke[i].dx, singleStroke[i].dy); }
             canvas.drawPath(path, tracePaint);
          } else {
              canvas.drawCircle(singleStroke[0], tracePaint.strokeWidth / 4, circlePaint);
          }
       }
    }
  }
  ...
  @override
  bool shouldRepaint(covariant TracingPainter oldDelegate) { /* ... codice invariato ... */
